...
import { deleteOrderByPenjualanIDAdmin, fetchAllPenjualanAPIAdmin, fetchDetailPenjualanByIdAdmin } from '../../../../store/endpoint/penjualan/penjualanEnd';
...
export default function AllPenjualanPagePetugas() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const data = await fetchAllPenjualanAPIAdmin();
                setPenjualan(data);
            } catch (error) {
    ...
    const toggleDetail = async (penjualanId) => {
        if (expandedPenjualanId === penjualanId) {
            ...
            setDetailPenjualan(null);
        } else {
            try {
                const detail = await fetchDetailPenjualanByIdAdmin(penjualanId);
                console.log(detail);
                setDetailPenjualan(detail);
                setExpandedPenjualanId(penjualanId);
            } catch (error) {
    ...
    const handleDelete = async () => {
        if (selectedPenjualanId) {
            try {
                await deleteOrderByPenjualanIDAdmin(selectedPenjualanId);
    ...
    const groupedProducts = detailPenjualan?.detailPenjualan.reduce((acc, item) => {
        ...
        if (existingProduct) {
            existingProduct.JumlahProduk += item.JumlahProduk;
        } else {
            acc.push({ ...item });
        }
        return acc;
